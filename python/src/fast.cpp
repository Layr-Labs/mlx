...
void init_fast(nb::module_& parent_module) {
  ...
  m.def(
      ...
      "stream"_a = nb::none(),
      nb::sig(
          "def scaled_dot_product_attention(q: array, k: array, v: array, *, scale: float, mask: Union[None, str, array] = None, sinks: Optional[array] = None, force_fused: bool = False, stream: Union[None, Stream, Device] = None) -> array"),
      R"pbdoc(
        ...
        * ``D``: The per-head dimension.
        ...
        Args:
            ...
            force_fused (bool, optional): If ``True``, use a fused kernel
               regardless of the builtin heuristics and raise an error when no
               fused kernel supports the configuration. This can reduce memory
               use at the cost of performance. Default: ``False``.
