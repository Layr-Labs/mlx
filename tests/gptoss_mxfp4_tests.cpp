...
struct ScopedPrefillTile {
  ...
  explicit ScopedPrefillTile(const char* value) {
    if (const auto* old = std::getenv("MLX_GPTOSS_MXFP4_PREFILL_TILE"))
      previous = old;
    setenv("MLX_GPTOSS_MXFP4_PREFILL_TILE", value, 1);
  }
  ~ScopedPrefillTile() {
    if (previous)
      setenv("MLX_GPTOSS_MXFP4_PREFILL_TILE", previous->c_str(), 1);
    else
      unsetenv("MLX_GPTOSS_MXFP4_PREFILL_TILE");
  }
};
} // namespace

TEST_CASE(
    "gptoss mxfp4 prefill tiles preserve expert boundaries and partial rows") {
  constexpr int E = 32, N = 2880, K = 2880;
  auto packed = reshape(
      multiply(
          arange(E * N * K / 8, uint32, Device::gpu),
          array(uint32_t{2654435761}),
          Device::gpu),
      {E, N, K / 8});
  ...
  for (const auto dtype : {float32, bfloat16}) {
    for (int rows : {128, 131, 257}) {
      ...
      auto x = astype(
          reshape(
              sin(arange(rows * K, float32, Device::gpu) * array(0.017f),
                  Device::gpu),
              {rows, 1, K}),
          dtype,
          Device::gpu);
      ...
      for (const auto* name : {"m32n32k32"}) {
        ...
        const auto delta =
            abs(astype(actual, float32) - astype(reference, float32));
        const float worst = max(delta).item<float>();
        const float magnitude =
            max(abs(astype(reference, float32))).item<float>();
        CHECK(std::isfinite(worst));
        CHECK(
            worst <=
            (dtype == float32 ? 1e-4f : 0.016f) * std::max(1.0f, magnitude));
      }
    }
  }
}
...
struct ScopedDecodeFastTail {
  ...
  explicit ScopedDecodeFastTail(const char* value) {
    if (const auto* old = std::getenv("MLX_GPTOSS_MXFP4_DECODE_FAST_TAIL"))
      previous = old;
    setenv("MLX_GPTOSS_MXFP4_DECODE_FAST_TAIL", value, 1);
  }
  ~ScopedDecodeFastTail() {
    if (previous)
      setenv("MLX_GPTOSS_MXFP4_DECODE_FAST_TAIL", previous->c_str(), 1);
    else
      unsetenv("MLX_GPTOSS_MXFP4_DECODE_FAST_TAIL");
  }
};
} // namespace
      ...
      for (bool tail_edges_only : {true, false}) {
        ...
        for (int row = 0; row < batch; ++row) {
          if (tail_edges_only) {
            ...
            for (int k = 0; k < K; ++k)
              values[row * K + k] = std::sin((row * K + k) * 0.017f);
          }
        }
        auto x =
            astype(array(values.data(), {batch, 1, K}), dtype, Device::gpu);
        array reference(0.0f);
        {
