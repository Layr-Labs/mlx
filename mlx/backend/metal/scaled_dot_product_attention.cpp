...
std::tuple<bool, std::string> has_fused_kernel(
  ...
  if (query_sequence_length > 8) {
    ...
    if (!supported_head_dim) {
      msg << "the full attention kernel supports head dims {64, 80, 128, "
             "256} with matching query/value head dims; got query head dim "
          << query_head_dim << " and value head dim " << value_head_dim << ".";
      return {false, msg.str()};
    }
    if (query_head_dim == 256 && q.dtype() != float16 &&
        q.dtype() != bfloat16) {
      msg << "the full attention kernel for head dim 256 supports only "
             "float16 and bfloat16; the current BQ=32, BK=16 float32 "
             "geometry requires 53760 bytes of threadgroup memory, exceeding "
             "the 32 KiB limit.";
      return {false, msg.str()};
    }
    if (has_mask && !has_arr_mask &&
        !(query_sequence_length <= key_sequence_length && do_causal)) {
      msg << "the full attention kernel with a causal mask requires the query "
             "sequence to be no longer than the key sequence; got query "
             "length "
    ...
    const bool supported_head_dim = query_head_dim == value_head_dim &&
        (query_head_dim == 64 || query_head_dim == 96 ||
         query_head_dim == 128 || query_head_dim == 256);
    if (!supported_head_dim) {
      msg << "the vector attention kernel supports matching query/value head "
             "dims {64, 96, 128, 256}; got query head dim "
          << query_head_dim << " and value head dim " << value_head_dim << ".";
      return {false, msg.str()};
    }
    ...
    if (query_sequence_length * gqa_factor > 32) {
      msg << "the vector attention kernel requires the query length times the "
             "GQA factor to be at most 32; got query length "
...
bool ScaledDotProductAttention::use_fallback(
  ...
  // D256 full attention is opt-in because its bounded-memory fused kernel can
  // be slower than the composed path when memory pressure is low.
  return q.shape(2) > 8 && q.shape(-1) == 256;
}
